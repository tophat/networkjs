...
describe('Network', () => {
    ...
    describe('on', () => {
        it('throws error on bad input', () => {
            expect(() => {monitor.on({}, jest.fn)}).toThrow(Error)
        })
        it('throws error on string not in NetworkStatuses', () => {
            expect(() => {monitor.on('INVALID', jest.fn)}).toThrow(Error)
        })
        it('adds an EventListener to the monitor eventEmitter', () => {
            monitor.on('online', jest.fn)
            expect(monitor.eventEmitter.addEventListener).toHaveBeenCalledTimes(1)
        })
    })
    ...
    describe('all', () => {
        it('Adds an EventListener for each NetworkStatus', () => {
            monitor.all(jest.fn)
            expect(monitor.eventEmitter.addEventListener).toHaveBeenCalledTimes(NetworkStatuses.length)
        })
    })
    ...
    describe('serviceError', () => {
        it('Calls handleError from the service monitor', () => {
            ...
            expect(monitor.monitors[Monitor.SERVICE].handleError).toHaveBeenCalled()
        }) 
    })

    describe('pause', () => {
        it('calls pause on the given monitor', () => {
            let pauseSpy = jest.spyOn(monitor.monitors[Monitor.SERVICE], 'pause')
    ...
    describe('resume', () => {
        it('calls resume on the given monitor', () => {
            ...
            let resumeSpy = jest.spyOn(monitor.monitors[Monitor.SERVICE], 'resume')
